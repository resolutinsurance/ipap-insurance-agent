...
const AgentRemotePremiumFinancing = () => {
  ...
  const currentStep = (Number(searchParams.get('step') || '1') || 1) as
    | 1
    | 2
    | 3
    ...
    return (
      <div className="flex h-full items-center justify-center">
        <p className="text-muted-foreground text-sm">
          Missing information for remote premium financing. Please return and
          start again.
        </p>
      </div>
    )
  ...
  const handleGenerateLink = async () => {
    ...
    if (!loanData) {
      toast.error(
        'Loan data is missing. Please complete the loan calculation step.',
      )
    ...
    try {
      ...
      console.error(
        'Failed to set up premium financing for remote verification',
        error,
      )
    }
  }
    ...
    <section>
      <Card>
        <CardContent className="flex flex-col gap-6 md:flex-row">
          <WidthConstraint className="mx-0 flex flex-col gap-6 p-5 sm:gap-10 lg:flex-row">
                ...
                paymentVerification.loanData?.totalPaid != null ? (
                  <RepaymentSchedulePreviewStep
                    paymentData={{
                      ...
                      totalPaid: String(
                        paymentVerification.loanData?.totalPaid ?? 0,
                      ),
                      loanAmount: String(
                        paymentVerification.loanData?.loanAmount ?? 0,
                      ),
                      noofInstallments: Number(
                        paymentVerification.loanData?.noofInstallments ?? 0,
                      ),
                      paymentFrequency:
                        paymentVerification.loanData?.paymentFrequency ||
                        'monthly',
                    }}
                    onNext={() => goToStep(3)}
                    onCancel={() => goToStep(1)}
