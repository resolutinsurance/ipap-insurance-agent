...
import { MOBILE_MONEY_METHODS, MainProductQuoteType, USER_TYPES } from "@/lib/constants";
import {
  QuotePaymentRequest,
...
interface UseSubmitQuotePaymentParams {
...
export function useSubmitQuotePayment({
  premiumAmount,
  customerId,
  selectedCompanyId,
  selectedQuoteId,
  ...
  const submitPayment = React.useCallback(async () => {
    ...
    try {
      ...
      // Debug: Log the state to help diagnose loanData issues
