...
export function PaymentDetailsStep({
  ...
  const bankTransferBanks = React.useMemo(() => {
    ...
    return BANK_CODES.filter(
      (bank) => !mobileMoneyCodes.includes(bank.assigned_code),
    )
  }, [])
  ...
  const handleMobileMoneyProviderChange = (value: string) => {
    const selectedMethod = MOBILE_MONEY_METHODS.find(
      (m) => m.methodName === value,
    )
    if (selectedMethod) {
  ...
  const handleSubmit = () => {
    ...
    if (method === PAYMENT_METHODS.BANK_TRANSFER) {
      ...
      const selectedBank = BANK_CODES.find(
        (b) => b.assigned_code === methodCode,
      )
      if (!selectedBank) {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-2 text-gray-600">
          Enter your payment method and account information to complete the
          payment
        </p>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <Select value={method} onValueChange={handleMethodChange}>
            ...
            <SelectContent>
              <SelectItem value={PAYMENT_METHODS.MOBILE_MONEY}>
                Mobile Money
              </SelectItem>
        ...
        {method === PAYMENT_METHODS.MOBILE_MONEY && (
          <div className="space-y-2">
            ...
            <Select
              value={methodName}
              onValueChange={handleMobileMoneyProviderChange}
            >
            ...
            <Select value={methodCode} onValueChange={handleBankChange}>
              ...
              <SelectContent>
                {bankTransferBanks.map((bank) => (
                  <SelectItem
                    key={bank.assigned_code}
                    value={bank.assigned_code}
                  >
                    {bank.bank_name}
                  </SelectItem>
                ))}
          ...
          {accountNumber.trim().length >= 10 &&
            ...
            !validateAccount.isPending && (
              <p className="text-sm text-amber-600">
                Account validation failed. Please check your account number and
                try again.
              </p>
            )}
        </div>
        <PaymentSummary
          ...
          actualProcessingFee={
            loanData?.actualProcessingFee ||
            loanCalculationData?.actualProcessingFee ||
            0
          }
          initialDeposit={
